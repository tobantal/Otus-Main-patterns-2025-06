...
TEST(InvalidateCacheHandlerTest, ClearsAllCache) {
    auto cache = std::make_shared<MockRulesCache>();
    InvalidateCacheHandler handler(cache);

    // создаём реальные объекты IRequest/IResponse
    SimpleRequest request("DELETE", "/cache/invalidate", "", "127.0.0.1", 8080, {});
    SimpleResponse response;

    // ожидаем вызов clear()
    EXPECT_CALL(*cache, clear()).Times(1);
