...
TEST_F(AdapterCodeGeneratorTest, GenerateBasicAdapter) {
    ...
    EXPECT_NE(result.find("UObject* obj_;"), std::string::npos);
    EXPECT_NE(result.find("explicit MovableAdapter(UObject* obj)"), std::string::npos);
...
TEST_F(AdapterCodeGeneratorTest, GenerateGetterMethod) {
    ...
    EXPECT_NE(result.find("IoC::resolve<Vector>(\"IMovable:Position.get\", obj_)"), std::string::npos);
}
...
TEST_F(AdapterCodeGeneratorTest, GenerateAdapterWithCustomImplementation) {
    ...
    EXPECT_NE(result.find("MovableImplementations::calculateVelocity(obj_)"), std::string::npos);
...
TEST_F(AdapterCodeGeneratorTest, GenerateAdapterForEmptyInterface) {
    ...
    EXPECT_NE(result.find("class AutoGenerated::EmptyAdapter"), std::string::npos);
    EXPECT_NE(result.find("public IEmpty"), std::string::npos);
    
    // Но методов (кроме конструктора) быть не должно
    EXPECT_EQ(result.find("override"), std::string::npos);
}
...
TEST_F(AdapterCodeGeneratorTest, GenerateAdapterWithComplexParameters) {
    ...
    // Проверяем IoC вызов
    EXPECT_NE(result.find("IoC::resolve(\"IComplexInterface:processData\""), std::string::npos);
    EXPECT_NE(result.find("obj_, objects, predicate, flags"), std::string::npos);
}
...
TEST_F(AdapterCodeGeneratorTest, HandleSpecialCharactersInNames) {
    ...
    EXPECT_NE(result.find("class AutoGenerated::Special_InterfaceAdapter"), std::string::npos);
}
